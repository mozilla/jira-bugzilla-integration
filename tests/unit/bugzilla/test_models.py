...
def test_patch_attachment_returns_valid_phabricator_url(bug_factory):
    ...
    assert (
        bug.attachment.phabricator_url(base_url=base_url)
        == "https://phabricator.services.mozilla.com/D262912"
    )
...
def test_patch_attachment_with_unexpected_file_name_returns_no_phabricator_url(
    bug_factory,
):
    bug = bug_factory.build(
        with_attachment=True,
