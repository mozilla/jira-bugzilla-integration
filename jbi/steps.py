...
def sync_dependencies(
    ...
    *,
    ...
    """
    depends_on_changed = (
        not context.event.changes or "depends_on" in context.event.changed_fields()
    )
    blocks_changed = (
        not context.event.changes or "blocks" in context.event.changed_fields()
    )

    if context.event.changes and not depends_on_changed and not blocks_changed:
        return (StepStatus.NOOP, context)

    if not context.event.changes and not context.bug.depends_on and not context.bug.blocks:
        return (StepStatus.NOOP, context)
    ...
    def _sync_field(bug_ids: list[int], *, action: str, primary_is_blocked: bool) -> None:
    ...
    if depends_on_changed:
        if context.event.changes:
            removed = _parse_changed_bug_ids(context.event.changes, "depends_on", "removed")
            if removed:
                _sync_field(removed, action="delete", primary_is_blocked=True)
        if context.bug.depends_on:
            _sync_field(context.bug.depends_on, action="create", primary_is_blocked=True)
